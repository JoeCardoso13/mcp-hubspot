...
MCP server providing HubSpot API functionality via FastMCP.
...
├── api_client.py  # Async HTTP client for HubSpot API
└── api_models.py  # Pydantic models for API responses
```

## Critical

- Package name: `@JoeCardoso13/hubspot` (npm-style scope, matches GitHub org)
- Manifest uses module execution: `python -m mcp_hubspot.server`
...
Access token configured via manifest `user_config`, not hardcoded:
```json
{
  "user_config": {
    "access_token": {
  ...
  "server": {
    "mcp_config": {
      "env": { "HUBSPOT_ACCESS_TOKEN": "${user_config.access_token}" }
    }
  }
}
```
...
| `src/mcp_hubspot/__init__.py` | `__version__` |
